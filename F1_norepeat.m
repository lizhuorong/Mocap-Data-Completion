...
[AN,repeatID] = checkrepeat(AN); %remove the repeated joints: 57=>45
...
% final
function [mae_PLOS1_joint,mae_PLOS1_frame,mae_xyz,A_GAP_pad] = compute_F1(A,B)
...
% % every single frame:from 61
% GT = GT(:,61:60+frame);
% A10 = A1(:,[1:60,60+frame+1:end]);
% alpha = A10*u([1:60,60+frame+1:end],:)*inv(u([1:60,60+frame+1:end],:)'*u([1:60,60+frame+1:end],:));
% A11 = alpha*u(61:60+frame,:)';
% Astar = A11 + mean1(61:60+frame,:)';% 57*ms_frame
...
% A_GAP((frame-1)*100+61:(frame-1)*100+60+frame,:)=Astar';

[n,ms_frame] = size(Astar);% n: 45, m: ms frame
...
if ~isempty(repeatID)
    A_GAP_pad = [A_GAP(:,1:15),A_GAP(:,13:15),A_GAP(:,16:24),A_GAP(:,22:24),...
    A_GAP(:,25:36),A_GAP(:,34:36),A_GAP(:,37:45),A_GAP(:,43:45)];% 4000x57
end
